...
layout  (location = 5) in vec2 outUV;
...
void main() {
    ...
    vec4 sampledNormal = texture(globalTextures2D[normalSamplerIndex], outUV);
    sampledNormal = normalize(sampledNormal * 2.0 - 1.0);
    sampledNormal = vec4(normalize(TBNMatrix * sampledNormal.xyz), 1.0); 

    outColor = sampledNormal;
