...
void main()
{
	...
	Vertex v1 = pc.vertexBufferAddress.vertices[tri.x];
	Vertex v2 = pc.vertexBufferAddress.vertices[tri.x];
	...
	float ndotL = max(dot(lightVector, normal), 0.2);

	vec3 worldPos = gl_WorldRayOriginEXT + gl_WorldRayDirectionEXT * gl_HitTEXT;

	// irrad.x, irrad.y, irrad.z, localPos.z
	hitValue = vec4(ndotL, ndotL, ndotL, worldPos.z);
}