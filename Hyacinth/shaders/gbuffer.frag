...
layout	(location = 7) in vec4 inUV;
...
void main() {
    ...
    vec4 sampledColor = texture(globalTextures2D[m.baseColorIndex], inUV.xy);
    vec4 metalRough = texture(globalTextures2D[m.metalRoughIndex], inUV.xy);

    vec3 N = texture(globalTextures2D[m.normalIndex], inUV.xy).xyz;
	...
	// float shadow = texture(lightMap, inUV.zw).r / 30000.0;
