...
void vkimageutils::createImageView(VkDevice& device, VulkanImage& image, VkImageAspectFlags aspectFlags) {
...
VulkanImage vkimageutils::createImage(DeviceContext& ctx, VkExtent3D size, VkFormat format, VkImageUsageFlags usage, VkSampleCountFlagBits numSamples, bool mipped) {
	...
	vkimageutils::createImageView(*ctx.device, newImage, aspectFlag);
...
VulkanImage vkimageutils::createImage(DeviceContext& ctx, void* data, VkExtent3D size, VkFormat format, VkImageUsageFlags usage, bool mipped) {
	...
	vkdeviceutils::destroyBuffer(*ctx.allocator, uploadBuffer);
	vmaSetAllocationName(*ctx.allocator, newImage.imageAllocation, "texture_image");
	return newImage;
}

void vkimageutils::createImageSampler(VkDevice& device, VulkanImage& image) {
